...
function print_post($post){ ?>
	...
	if ( get_field('event_date2') ) {
		...
		$full_event_date = date_format( $date, "m.d.Y" );
	}
	...
	<div class="a-post" data-carousel="<?php if(isset($has_carousel) && $has_carousel != 0){ echo $has_carousel; } ?>">
		...
		<div class="the-text">
			...
			<?php if ( get_field('event_date2') ) { ?>
				<span class="post-cat"><?php echo $post_cat; ?>&nbsp;&nbsp;&nbsp;/&nbsp;&nbsp;&nbsp;</span>
				<span class="post-date"><?php echo $full_event_date; ?></span>
			<?php } else { ?>
				<span class="post-cat"><?php echo $post_cat; ?></span>
			<?php } ?>
			...
			$posts->the_post(); ?>
				...
				<?php
					...
					if(get_field('event_date2')){
						...
						$date2 = date_create( $event_date );
						$full_event_date2 = date_format( $date2, "m.d.Y" );
				...
				<?php } elseif($post_cat == 'Posts' || $post_cat == 'Deals'){ ?>
					...
					<div class="a-modal other">
						...
						<section class="right">
							...
							<div class="info-bar">
								<?php if ( get_field('event_date2') ) { ?>
									<span class="date heading">Date:</span><span class="date mr20"><?php echo $full_event_date2; ?></span>
								<?php } ?>
								<span class="date heading">Author:</span><span class="date"><?php the_author(); ?></span>
							</div>
							<div class="the-content">
								<div class="inner">
									<?php if(get_field('post_sub')){ ?>
